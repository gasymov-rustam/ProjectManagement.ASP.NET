...
internal sealed class DataInitializer : IHostedService
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        if (cancellationToken.IsCancellationRequested)
        {
            _logger.LogInformation(
                "The application is shutting down. The data initialization is skipped."
            );
    ...
    public Task StopAsync(CancellationToken cancellationToken) =>
        cancellationToken.IsCancellationRequested ? Task.CompletedTask : null;
}

internal sealed class DatabaseInitializer<TContext> : IHostedService
